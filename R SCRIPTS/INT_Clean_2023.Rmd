...
# Import excel sheet
raw.data <- read_excel("Coral growth - experiments_Intertidal 2022.xlsx")
...
# Import excel sheet
clean.data.1 <- read_excel("INT_Cleaning experiment.xlsx", sheet = 1)
...
lme.sgr.2r.S  <- lme(SGR ~ Treatment*Species*Date, method = "ML", na.action = na.omit, random = ~1 |
                       Position, data = sgr.data.1, weights = varIdent(form = ~1 | Species))

## Model output
Anova(lme.sgr.2r.S) # Every variable is (highly) significant, Two- and three-way interactions are as well highly significant
